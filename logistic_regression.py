...
def generate_ellipsoid_clusters(distance, n_samples=100, cluster_std=0.5):
    ...
    X2[:, 0] += distance
    X2[:, 1] -= distance  # Shift along y = -x by subtracting from y
...
def do_experiments(start, end, step_num):
    ...
    for i, distance in enumerate(shift_distances, 1):
        ...
        plt.text(x_min + 0.1, y_max - 1.0, equation_text, fontsize=14, color="black", ha='left', bbox=dict(facecolor='white', edgecolor='black', boxstyle='round,pad=0.3'))
        plt.text(x_min + 0.1, y_max - 1.5, margin_text, fontsize=14, color="black", ha='left', bbox=dict(facecolor='white', edgecolor='black', boxstyle='round,pad=0.3'))
